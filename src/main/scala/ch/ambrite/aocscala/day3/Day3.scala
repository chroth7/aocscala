...
object Day3 {
  ...
  def findBestIntersectionSteps(wire1: Wire, wire2: Wire): Int = {
    val allIntersections: Seq[(Int, Int)] = wire1.init intersect wire2.init
    val wire1Rev = wire1.reverse
    val wire2Rev = wire2.reverse
    val steps: Seq[Int] = allIntersections map (intersection => (wire1Rev indexOf intersection) + (wire2Rev indexOf intersection))
    
    steps min
  }

  def day3_1(wireString1: String, wireString2: String): Int = findBestIntersectionDistance(makeWire(wireString1), makeWire(wireString2))
  def day3_2(wireString1: String, wireString2: String): Int = findBestIntersectionSteps(makeWire(wireString1), makeWire(wireString2))
}
